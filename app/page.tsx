...
export default function Home() {
  ...
  const getInitialLayout = () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      return {
        ...
        leftColSpan: window.innerWidth < 1024 ? 1 : 3,
        rightColSpan: window.innerWidth < 1024 ? 0 : 2,
        totalCols: window.innerWidth < 1024 ? 1 : 5,
        showRightColumn: window.innerWidth >= 1024
      }
    }
  ...
  useEffect(() => {
    ...
    const calculateDesktopLayout = (viewport) => {
      ...
      // Estimate total content height more accurately
      const estimatedContentHeight =
        60 + // Header
        40 + // Contact 
        120 + // Experience (2 items)
        80 + // Education 
        60 + // Skills
        (projects.length * 80) // Projects (more realistic estimate)

      // More aggressive scaling to fit all content
      let scale = Math.max(0.5, Math.min(1.2, availableHeight / estimatedContentHeight))

      // Better breakpoint logic for different screen sizes
      let gridCols, leftColSpan, rightColSpan
      ...
      // Content-based grid columns for projects section with better scaling
      const projectGridCols = availableWidth < 1024 ? 1 :
        availableWidth < 1200 ? 1 :
          availableWidth < 1600 ? 2 :
            scale > 0.8 ? 3 : 2 // Reduce columns if we need more vertical space

      // More compact settings for better fit
      const scaleSettings = {
        small: { // scale < 0.6
          fontSize: 9, spacing: 6, showAllProjects: true, showSecondaryInfo: false
        },
        medium: { // scale < 0.8
          fontSize: Math.floor(11 * scale), spacing: Math.floor(12 * scale),
          showAllProjects: true, showSecondaryInfo: scale > 0.7
        },
        normal: { // scale >= 0.8
          fontSize: Math.floor(13 * scale), spacing: Math.floor(14 * scale),
          showAllProjects: true, showSecondaryInfo: true
        }
      }

      const settings = scale < 0.6 ? scaleSettings.small :
        scale < 0.8 ? scaleSettings.medium : scaleSettings.normal

      setLayout({
        fontSize: Math.max(9, Math.min(16, settings.fontSize)),
        spacing: Math.max(6, Math.min(20, settings.spacing)),
        padding: Math.max(12, Math.floor(20 * scale)),
        headerSize: Math.max(16, Math.floor(22 * scale)),
        sectionSpacing: Math.max(8, Math.floor(16 * scale)),
        itemSpacing: Math.max(3, Math.floor(6 * scale)),
        gridCols: projectGridCols,
        showAllProjects: settings.showAllProjects,
        showSecondaryInfo: settings.showSecondaryInfo,
        leftColSpan,
        rightColSpan,
        totalCols: gridCols,
        showRightColumn: rightColSpan > 0
      })
    }
  ...
  const DesktopLayout = memo(function DesktopLayout() {
    return (
      ...
      >
        <div
          className="grid h-full grid-transition auto-fit-container"
          style={{
            gap: `${layout.spacing}px`,
            gridTemplateColumns: layout.totalCols === 1
              ? '1fr'
              : `repeat(${layout.totalCols}, 1fr)`
          }}
        >
          {/* Left Column - Auto-fitting main content */}
          <div
            className="overflow-hidden flex flex-col responsive-column layout-transition"
            style={{
              gap: `${layout.sectionSpacing}px`,
              gridColumn: `span ${layout.leftColSpan}`
            }}
          >
            ...
            {/* Projects - Now with better auto-fit */}
          ...
          {/* Right Column - Only show when there's enough space */}
          {layout.showRightColumn && (
            <div
              className="overflow-hidden"
              style={{ gridColumn: `span ${layout.rightColSpan}` }}
            >
              <div
